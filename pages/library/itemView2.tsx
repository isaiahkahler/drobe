...
export class ItemView extends React.Component<ItemViewProps, ItemViewState> {
  constructor(props: ItemViewProps) {
    super(props);
    this.state = { item: null, editable: false };
  }
  //review: is the second param of 'get param' the default value?
  static navigationOptions = ({ navigation }) => {
    return {
      title: "Item",
      headerRight: navigation.getParam("editable", undefined) ? (
        <TouchableHighlight
  ...
  componentDidMount = async () => {
    this.props.navigation.setParams({ edit: this.editItem })
    if("item" in this.props.navigation.state.params){
      this.setState({ item: this.props.navigation.state.params.item });
    }
    if("editable" in this.props.navigation.state.params){
      this.setState({editable: this.props.navigation.state.params.editable})
    }
  };

  editItem = async () => {
    let {pageIndex, itemIndex} = await ItemManager.getItemIndexesAsync(this.state.item);
      this.props.navigation.navigate("Define", {
        data: {
          editMode: true,
          pageIndex: pageIndex,
          itemIndex: itemIndex,
          uri: Storage.getLibraryPhotosDirectory() + '/' + this.state.item.photoURI
        }
      });
  };
      ...
      [
        {
          text: "yes",
          onPress: async () => {
            await ItemManager.deleteItem(this.state.item);
    ...
    if (!!this.state.item) {
      return (
        <PageLayout scroll padding>
          ...
          <Image
            source={{ uri: Storage.getLibraryPhotosDirectory() + '/' + this.state.item.photoURI }}
          ...
          />
          <ShowValue
            name="total times worn:"
            type="uses"
            value={this.state.item.uses}
          />
          <ShowValue
          ...
          {this.state.editable ? (
            <TouchableHighlight
              ...
              style={[
                commonStyles.button,
                { backgroundColor: "#ff0000", marginTop: 5 }
              ]}
      ...
      return (
        <View>
          <Text>oh! something went wrong and you shouldn't be seeing this.</Text>
        </View>
      );
    }
  }
